...
- ✅ **Deletion System**: Implemented node deletion with confirmation dialogs and file cleanup in Supabase Storage
...
- `apps/web/src/store/nodes.store.ts` - Added setSelectedNodeIds method for selection management
- `apps/web/src/hooks/nodes/useNodeDrag.ts` - Enhanced with Konva integration, multi-node support, and database persistence
- `apps/web/src/components/nodes/FileNode.tsx` - Added selection indicators, multi-select support, loading states, and drag integration
- `apps/web/src/components/canvas/NodesLayer.tsx` - Integrated selection, deletion, and keyboard shortcuts
