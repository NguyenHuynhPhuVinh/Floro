...
jest.mock('../GridBackground', () => ({
  GridBackground: ({ type, size, color, opacity }: any) => (
    <div data-testid="grid-background" data-type={type} data-size={size} data-color={color} data-opacity={opacity}>
      Grid Background
    </div>
  ),
}));
...
jest.mock('../DotsBackground', () => ({
  DotsBackground: ({ type, size, color, opacity }: any) => (
    <div data-testid="dots-background" data-type={type} data-size={size} data-color={color} data-opacity={opacity}>
      Dots Background
    </div>
  ),
}));
...
describe('CanvasBackground', () => {
  ...
  it('does not render when background type is none', () => {
    // Mock none background type
    jest.doMock('../../../store/settings.store', () => ({
      useSettingsStore: () => ({
        canvas: { ...mockCanvasSettings, backgroundType: 'none' },
      }),
    }));
  ...
  it('uses correct color based on theme', () => {
    ...
    const gridBackground = screen.getByTestId('grid-background');
    // Light theme should use gray color
    expect(gridBackground).toHaveAttribute('data-color', '#9CA3AF');
  });
  ...
  it('has correct container styling', () => {
    ...
    expect(backgroundContainer).toHaveClass('absolute', 'inset-0', 'pointer-events-none');
